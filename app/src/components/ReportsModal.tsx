...
import { X, Flame, ChevronLeft, ChevronRight, ArrowLeft, Dumbbell, Footprints, TrendingUp, TrendingDown, Calendar } from './BroskyIcon';
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-100 flex gap-2">
          {!showResult ? (
            ...
            <>
              <button 
                onClick={() => setShowResult(false)}
                className="flex-1 py-3.5 bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-black rounded-xl transition-all cursor-pointer"
              >
                Изменить период
              </button>
              <button 
                onClick={onClose}
                className="flex-1 py-3.5 bg-gym-accent hover:bg-gym-accent/90 text-white text-sm font-black rounded-xl transition-all shadow-md shadow-gym-accent/20 cursor-pointer"
              >
                Готово
              </button>
            </>
