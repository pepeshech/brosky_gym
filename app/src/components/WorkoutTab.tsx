...
import {
  Plus, Trash2, CheckCircle, ChevronDown, ChevronUp, Pencil, Check, X, Dumbbell, ClipboardList, Library, Trophy, ArrowRight, History, ChevronLeft, ChevronRight, Clock, Search, Layers, TrendingUp, Muscle } from './BroskyIcon';
...
const buildInitialLog = (
  ...
  const sets: SetLog[] = Array.from({ length: tmplEx.sets }, (_, i) => ({
    setIndex: i,
    reps: defaultReps,
    weight: lastLog?.sets?.[i]?.weight ?? lastLog?.weight ?? 0,
    isCompleted: false,
  }));
  ...
  useEffect(() => {
    ...
    template.exercises
      .forEach((tmplEx) => {
        ...
        initLogs[tmplEx.exerciseId] = buildInitialLog(tmplEx, lastLog);
      });
    const timer = setTimeout(() => {
      setLogs(initLogs);
      setSaved(false);
    ...
    return () => clearTimeout(timer);
  }, [selectedTemplateId, sessionDate, workoutTemplates, template, workoutSessions]);

  const handleSetChange = (exId: string, setIdx: number, field: 'weight' | 'reps' | 'rpe', val: number) => {
    setLogs((prev) => ({
      ...prev,
      [exId]: {
        ...prev[exId],
        sets: prev[exId].sets.map((s) =>
          s.setIndex === setIdx ? { ...s, [field]: val } : s
        ),
      },
    }));
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {orderedExercises.map((tmplEx) => {
        ...
        // Рассчитаем автопрогрессию нагрузок
        const recommendation = (() => {
          if (!lastSessionLog || !lastSessionLog.sets || lastSessionLog.sets.length === 0) {
            return { text: "Первая тренировка с этим упражнением. Начните с комфортного веса.", isIncrease: false };
          }
          
          // Проверяем, были ли все подходы выполнены успешно в прошлый раз
          const allCompleted = lastSessionLog.sets.every(s => s.isCompleted);
          const targetReps = parseRepsDefault(tmplEx.reps);
          const allMetTargetReps = lastSessionLog.sets.every(s => s.reps >= targetReps);
          
          if (allCompleted && allMetTargetReps) {
            const maxPastWeight = Math.max(...lastSessionLog.sets.map(s => s.weight));
            const newWeight = Math.round(maxPastWeight * 1.025 * 2) / 2 || maxPastWeight + 0.5;
            const increase = newWeight - maxPastWeight;
            return {
              text: `Прогрессия: +${increase} кг (до ${newWeight} кг), так как прошлый план выполнен!`,
              isIncrease: true,
              weight: newWeight
            };
          } else {
            const maxPastWeight = Math.max(...lastSessionLog.sets.map(s => s.weight));
            return {
              text: `Повтор: закрепить вес ${maxPastWeight} кг и закрыть целевые повторения (${targetReps}).`,
              isIncrease: false,
              weight: maxPastWeight
            };
          }
        })();
          ...
          >
            ...
            <div className="px-5 py-4 flex items-start justify-between" style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
              <div className="flex-1">
                ...
                {workoutStarted && (
                  <div className={`mt-2.5 px-3 py-1.5 rounded-xl text-[10px] font-bold border flex items-center gap-1.5 w-fit shadow-xs leading-none select-none ${
                    recommendation.isIncrease 
                      ? 'bg-emerald-50/60 border-emerald-100/50 text-emerald-600' 
                      : 'bg-blue-50/60 border-blue-100/50 text-blue-500'
                  }`}>
                    {recommendation.isIncrease ? (
            ...
            <div className="px-3 sm:px-5 py-3 space-y-2 w-full">
              ...
              {log.sets.map((set) => {
                const setRM = set.weight > 0 && set.reps > 0 ? calcEpley1RM(set.weight, set.reps) : null;
                    ...
                    <div className="flex flex-col items-center justify-center min-w-0">
                      ...
                      {set.isCompleted ? (
                        <div className="flex flex-col items-center gap-0.5 animate-fadeIn">
                          <span className="text-[8px] font-bold text-gray-400 select-none">RPE</span>
                          <div className="flex gap-0.5">
                            {[7, 8, 9, 10].map(val => (
                              <button
                                key={val}
                                onClick={() => handleSetChange(tmplEx.exerciseId, set.setIndex, 'rpe', val)}
      ...
      {template && workoutStarted && (
        <div className="glass-panel rounded-2xl p-5 space-y-3">
          ...
          <button
            onClick={handleSave}
            disabled={saved}
            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-white transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
