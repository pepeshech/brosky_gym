...
import { Droplet, Trash2, Calendar, Plus, Sparkles, RefreshCw, ChevronLeft, ChevronRight, Search, Footprints, LoaderPulse, BookOpen, Settings, Target } from './BroskyIcon';
import { DatePicker } from './DatePicker';
import { useDialog } from './DialogProvider';
import type { NutritionPreset } from '../types';
...
export const NutritionTab: React.FC = React.memo(() => {
  ...
  // Генерация планов питания с учетом динамического тоннажа тренировки
  const plans = useMemo(() => {
    return generateDietPlans(activeProfileForCalc as any, totalVolumeToday);
  }, [activeProfileForCalc, totalVolumeToday]);

  // Определение целей на день (тренировка / отдых)
  const hasWorkout = workoutSessions.some(s => s.date === logDate);
  ...
  const target = useMemo(() => {
    ...
    if (preset.type === 'static') {
      ...
      return {
        ...
        water: preset.waterMl ?? autoTarget.water,
  ...
  }, [selectedPresetId, presets, autoTarget, activeProfileForCalc]);
            ...
            <div className="md:col-span-1 flex flex-col gap-5">
              ...
              <div className="glass-card rounded-2xl p-4 flex flex-col justify-between h-[178px] relative group border border-gym-border/40 bg-white/80 shadow-md">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-gray-400 mt-1 font-semibold">План: {target.water} мл</div>
                </div>
                  ...
                  <div className="relative search-container-el">
                    ...
                    <div className="relative flex items-center">
                      <input
                        ...
                        className="w-full bg-white/70 border border-gym-border rounded-xl pl-9 pr-3 py-2 text-sm text-gray-800 focus:outline-none focus:border-gym-accent"
                      />
                      {isSearchingApi ? (
