...
interface AnatomyModelProps {
  ...
  mode?: 'filter' | 'heatmap';
  onModeChange?: (mode: 'filter' | 'heatmap') => void;
  weeklyLoads?: Record<string, number>;  // Количество выполненных подходов за 7 дней
}
          ...
          <div className="h-7 flex items-center justify-center">
            {hovered ? (
              <span
                ...
                style={{
                  background: mode === 'heatmap' ? `${getHeatmapColor(hovered)}15` : `${MUSCLE_COLORS[hovered]}12`,
                  color: mode === 'heatmap' ? getHeatmapColor(hovered, true) : MUSCLE_COLORS[hovered],
                  borderColor: mode === 'heatmap' ? `${getHeatmapColor(hovered)}35` : `${MUSCLE_COLORS[hovered]}35`,
                  boxShadow: `0 2px 8px ${mode === 'heatmap' ? getHeatmapColor(hovered) : MUSCLE_COLORS[hovered]}12`,
                }}
              >
                {hovered} {mode === 'heatmap' && `: ${getStatusExplanation(hovered)}`}
              </span>
            ) : selectedFilter ? (
