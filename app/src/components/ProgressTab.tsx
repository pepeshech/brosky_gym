...
import { calculateLBM, generateDietPlans } from '../utils/formulas';
import {
  AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine,
  ComposedChart, Line, Bar, Legend
} from 'recharts';
import { Plus, Trash2, TrendingDown, TrendingUp, Pencil, Check, X, Settings, ChevronDown, ChevronUp, Layers, Dumbbell, Activity, AlertTriangle } from './BroskyIcon';
...
export const ProgressTab: React.FC = React.memo(() => {
  ...
  const handleCalculateBodyFat = (e: React.FormEvent) => {
    ...
    if (calcGender === 'male') {
      ...
      const density = 1.0324 - 0.19077 * Math.log10(waistVal - n) + 0.15456 * Math.log10(h);
      const fatPercent = (495 / density) - 450;
      setCalcResult(Math.round(Math.max(2, Math.min(60, fatPercent)) * 10) / 10);
    } else {
      if (isNaN(hipsVal)) {
        setErrorText('Для женщин необходим обхват бедер.');
        return;
      }
      if ((waistVal + hipsVal) <= n) {
        setErrorText('Сумма обхватов талии и бедер должна быть больше обхвата шеи.');
        return;
      }
      const density = 1.29579 - 0.35004 * Math.log10(waistVal + hipsVal - n) + 0.22100 * Math.log10(h);
      const fatPercent = (495 / density) - 450;
      setCalcResult(Math.round(Math.max(5, Math.min(70, fatPercent)) * 10) / 10);
    }
    setErrorText(null);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {subTab === 'metrics' && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm text-white shadow-sm transition-all cursor-pointer btn-interactive self-start sm:self-auto"
      ...
      <div className="flex gap-2 p-1 bg-white/50 border border-gym-border/60 rounded-2xl w-fit shadow-xs min-w-max">
        <button
            ...
            <form onSubmit={handleFullSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                <div className="relative z-20">
                   <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Дата</label>
                   <DatePicker
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Заметки</label>
                <textarea
                  name="notes"
          ...
          {addMode === 'partial' && (
            <form onSubmit={handlePartialSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="relative z-20">
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Параметр</label>
                  <select
                    value={partialKey}
                ...
                <div className="relative z-10">
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Значение</label>
                  <input
                    type="number"
                    step="any"
                    required
            ...
            <div className="space-y-4">
              <form onSubmit={handleCalculateBodyFat} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  {/* Рост */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Рост (см)</label>
                    <input
                      type="number"
                      step="any"
                      required
                  ...
                  {/* Вес */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Вес (кг)</label>
                    <input
                      type="number"
                      step="any"
                      required
                  ...
                  {/* Обхват шеи */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Обхват шеи (см)</label>
                    <input
                      type="number"
                      step="any"
                      required
                  ...
                  {/* Обхват талии */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Обхват талии (см)</label>
                    <input
                      type="number"
                      step="any"
                      required
                  ...
                  {calcGender === 'female' && (
                    <div>
                      <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Обхват бедер (см)</label>
                      <input
                        type="number"
                        step="any"
                        required
                      ...
                      >
                        ...
                        {editingKey === m.key ? (
                          ...
                          <div className="flex-1">
                            ...
                            {m.isVirtual && (
                              <span
                                className="ml-2 text-[10px] text-indigo-500 bg-indigo-50/70 border border-indigo-100 font-bold px-2 py-0.5 rounded-lg uppercase tracking-wider cursor-help flex inline-flex items-center gap-1"
                                title="Этот параметр вычисляется автоматически на основе других замеров и не вводится вручную. Например, ЧМТ (чистая мышечная масса) = вес − жировая масса."
                              >
                                <Activity size={11} className="text-indigo-500 fill-indigo-500/10" />
                                Вычисляемый
                              </span>
                            )}
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="bg-white/40 border-b border-gym-border text-gray-400 text-xs font-semibold uppercase tracking-wider">
                      ...
                      {tableCols.map(m => (
                        <th key={m.key} className="p-3 whitespace-nowrap">{m.name}</th>
                      ))}
                  ...
                  <tbody>
                    {progress.slice().reverse().map(p => (
                      <tr key={p.date} className="border-b border-gym-border/30 hover:bg-white/20 transition-colors">
                        <td className="p-3 font-semibold text-gray-600 font-mono text-xs">
                          {p.date.split('-').reverse().join('.')}
                        </td>
                        {tableCols.map(m => {
                          const val = p[m.key];
                          return (
                            <td key={m.key} className="p-3 text-gray-700 font-medium tabular-nums">
                              {val != null ? `${val} ${m.unit}` : <span className="text-gray-300">—</span>}
                            </td>
                          );
                        })}
                        <td className="p-3 text-xs text-gray-400 max-w-[150px] truncate" title={p.notes}>
                          {p.notes || '—'}
                        </td>
